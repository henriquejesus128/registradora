...
* [`src/mini_registradora/`](src/mini_registradora/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/mini_registradora/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `mini-registradora` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/mini_registradora/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns mini-registradora.example
  (:require [mini-registradora.config :as config])
